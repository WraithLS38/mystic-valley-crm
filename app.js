...
function initEventListeners() {
    ...
    document.getElementById('enrich-all-btn').addEventListener('click', enrichAllLeads);
        ...
        <tr data-id="${lead.id}">
            ...
            <td>
                <div class="action-buttons">
                    <button class="action-btn" onclick="enrichLead('${lead.id}')" title="Enrich Contact Info">
...
async function enrichAllLeads() {
    const leadsToEnrich = leads.filter(l => l.website && (!l.email || !l.phone));
    
    if (leadsToEnrich.length === 0) {
        showToast('No leads to enrich (all have contact info or no website)', 'info');
        return;
    }
    
    showToast(`Enriching ${leadsToEnrich.length} leads with contact info...`, 'info');
    
    let enriched = 0;
    for (const lead of leadsToEnrich) {
        try {
            await apiPost(`/leads/${lead.id}/enrich`, {});
            enriched++;
        } catch (error) {
            console.error(`Failed to enrich ${lead.company}:`, error);
        }
    }
    
    showToast(`Enriched ${enriched} leads with contact info`, 'success');
    await loadLeads();
}

async function enrichLead(leadId) {
    try {
        showToast('Extracting contact info from website...', 'info');
        const result = await apiPost(`/leads/${leadId}/enrich`, {});
        showToast(`Found: ${result.email || 'No email'}, ${result.phone || 'No phone'}`, 'success');
        await loadLeads();
    } catch (error) {
